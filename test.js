...
(function ($) {
    ...
    function aj() {
        ...
        jq({
               url: '/otn/dynamicJs/sfufpru', data: dobj, type: 'POST', success: function (data, textStatus) {
    ...
    $(document).ready(function () {
        (function () {
            ...
            if (null != form && form != 'undefined' && form.id == 'loginForm') {
                ...
                jQuery.extend({
                                  ajaxoverride: function () {
                                      var _ajax = $.ajax;
                                      $.ajax = function (opt) {
                                          if (opt.url && opt.url.indexOf('confirmPassenger/checkOrderInfo') >= 0) {
                                              var tmpdata = opt.data;
                                              var keyVlues = gc().split(':');
                                              tmpdata[keyVlues[0]] = encode32(bin216(Base32.encrypt(keyVlues[1], keyVlues[0])));
                                              opt.data = tmpdata;
                                          }
                                          var _opt = $.extend(opt, {});
                                          _ajax(_opt);
    ...
    function gc() {
        ...
        var keywordArr = [{key: ".enter_right", values: ["亲", "抢票", "助手"]}, {
            key: ".cx_form",
            values: ["点发车", "刷票"]
        }, {key: "#gridbox", values: ["只选", "仅选", "checkBox", "checkbox"]}, {key: ".enter_w", values: ["助手"]}];
